...
1. **Sub-second warm restart** for the common case (compose stack + deps installed, no project content changes). Live snapshots make this technically possible; bakeri.sh is the productization.
2. **A clean CI surface**: `bake run <command>` for one-shot jobs, `bake pr <pr-ref>` for PR-from-fork. No interactive UX required.
3. **No Docker on the host**. The whole point of running inside a VM is host isolation; bakeri.sh never spawns containers on the host.
4. **Idiomatic project files**. Users keep their existing `Dockerfile`, `docker-compose.yml`, `Gemfile`, `package.json`, etc. — no bakeri-specific config files.
...
- Replacing GitHub Actions / GitLab CI / Buildkite. bakeri.sh is a *runner* — something a CI pipeline calls into per job. Orchestration, secrets management, artifact upload, etc. stay with the existing CI platform.
...
```
docker-engine        cached, cold,  shared across all bakeri.sh projects
...
|---|---|---|---|---|
| `docker-engine` | cached | cold | none (pulled by deps) | `apk add docker docker-cli-compose`, `service docker start`. Key is a constant — single shared snapshot for all bakeri.sh projects. |
...
Post-MVP (once we have a `[memory]` declaration in bakeri.sh manifests, see below): `kind = "live"`. Snapshot captures running compose stack + Postgres/Redis page cache + db connection state. Restore loads via `aq --from-snapshot=...` with `-incoming file:memory.bin`, guest resumes mid-flight, `docker compose ps` reports all services running immediately. Target restoration time: <2 s wall clock.
...
bakeri.sh commands ship as their own plugin in the same pack. They are not implemented as part of any other plugin.

### `bake run [-- <command...>]`

Run one job in a fresh VM. Steps:

1. Walk the snapshot chain in the project (rlock framework, no bakeri-specific logic). Net result: a warm VM ready at the topmost layer.
...
The `--` is a convention to separate bakeri.sh flags (none yet) from the user command.
...
Run the same flow as `bake run`, but checkout an untrusted PR-from-fork before running the project's CI command. The PR ref can be:
...
- A patch file (`bake pr --patch /tmp/foo.patch`).

In all cases, the bakeri.sh host fetches the PR's commits into a local detached ref **on the host**, then pushes that ref into the VM via the standard `git` plugin. **No network access from the VM to the original fork.** The VM only sees what the host explicitly pushed — which means a malicious fork can't trigger network requests during checkout / dep install.

After the VM is provisioned + the project's deps are warm:

- `bake pr` checks for a `bake.yaml` or `.github/workflows/*.yml` (in priority order) to find the test command. Falls back to `bake run -- $(detect-test-cmd)` heuristics or to an explicit `--cmd` flag.
- Run the command, capture exit code + output.
- Tear down.

This is the highest-value piece of bakeri.sh for many users: isolated PR test runs that can't exfiltrate secrets, can't reach the internet to pivot, and start in under a second on warm cache.

### `bake snapshot [ls | rm | rebuild]`

Explicit management of the cache, mostly for debugging and reclaiming disk.

- `bake snapshot ls` — list cached layers per plugin, with sizes and last-used dates.
- `bake snapshot rm <plugin>[:<key>]` — drop a specific cache entry (or all entries for a plugin).
- `bake snapshot rebuild <plugin>` — force-rebuild a plugin's layer on the next `bake run`. Drops the entry and any descendants in the chain (otherwise they'd cache-hit but point at a defunct ancestor).

Implementation: thin wrapper around `rl cache` (which doesn't exist yet — see `rlock/TODO.md` "Snapshot analytics"). bakeri.sh's `bake snapshot ls` can ship before `rl cache stats` lands as a simple `du -sh` over `~/.local/share/aq/cache/*/*`.

## Benchmark targets

A real Rails 7 + Postgres 16 + Redis 7 + Sidekiq fixture project, average-sized (~100 K LOC, ~80 gems, ~500 npm packages, ~50 migrations). Numbers are wall-clock from `bake run` invocation to the user's command starting.
...
These are stretch targets to validate the architecture, not commitments. First measurements happen after the `mise` + `ruby-bundler` plugins ship and we benchmark `bake run` against a real fixture project (likely the rails-app fixture currently in `rlock/test/fixtures/rails-app/`).

## Distribution-level memory declarations

A project might pass `--memory=4G` to `aq` but bakeri.sh's plugin chain doesn't see that — `aq` is invoked once by rlock for the whole VM. So memory needs to be declared at the **distribution** or **project** level, not per plugin:
...
Option B — **project-level override file** (`bakeri.yaml` in repo root):

```yaml
memory: 6G
```

Read by bakeri.sh at the start of `bake run`; passed to `aq new`.

Option C — **`bake run --memory=NG`** explicit flag.
...
1. **Smoke fixture location.** `rlock/test/fixtures/rails-app/` currently has a minimal Rails+PG fixture used by framework integration tests. Whether to keep it in rlock (so framework integration tests cover docker-in-VM flows via bakeri.sh side-by-side checkout) or move to `bakeri.sh/test/fixtures/`. Recommendation: keep in rlock for now since rlock's integration_layered.sh already references it; revisit if rlock's test surface stops needing Docker.

2. **`bake pr` source resolution.** GitHub URL parsing requires either a `gh` dep or hand-rolled REST calls. Recommendation: shell out to `gh pr view --json` for GitHub URLs (covers 95% of use cases); document GitLab / Bitbucket as future plugin work.

3. **PR-from-trusted vs PR-from-fork.** Some teams want isolation only for forks. Add `bake pr --trust-internal` to skip the host-mediated git push for branches from the same repo (faster checkout, less isolation). Default: maximum isolation.

4. **Cache hit reporting.** Should `bake run` print which layers hit/missed by default? Recommendation: yes, one line per layer with timings, similar to `docker build` output. Helps users debug "why is my cache missing".

5. **`bake snapshot ls` format.** JSON vs table. Recommendation: human-readable table by default, `--json` flag for scripting.

6. **Concurrent `bake run` on the same project.** Two CI jobs hit the same warm cache simultaneously — does the qcow2 backing chain handle this? Yes (each VM gets its own overlay above the shared backing), but the framework should document this explicitly. Out of scope for the design but worth a smoke test.

## What's NOT in this spec (deferred)

- **`bake fanout` / parallel sharding.** aq already has `aq fanout` for parallel VM execution; surface it through bakeri.sh once the single-VM path is solid.
- **Cross-machine cache sync** (depot.dev pattern). Tracked in `rlock/TODO.md`.
- **Custom container runtimes** (podman, OrbStack). bakeri.sh runs vanilla Docker inside the Alpine VM; users wanting different runtimes can write their own plugin pack.
- **Network policy controls.** A workload running inside the VM can reach the internet via `10.0.2.2` user-mode NAT today. Stricter egress (allowlists, mitm logging) is a future bakeri.sh concern, not MVP.
- **CI-platform integrations** (GHA / GitLab / Buildkite YAML generators). bakeri.sh is the runner; integrations come once the runner is solid.
...
3. **`bake run` command** — productizes the cache-walk-and-exec flow.
4. **`docker-compose` kind = "live" flip** + memory declaration mechanism.
5. **`bake pr` command** — high-value, more involved.
6. **`bake snapshot ls / rm / rebuild`** — debug ergonomics.
